...
impl<'de> Visitor<'de> for ColorVisitor {
    ...
    {
        if value.len() != 7 {
            return Err(E::custom(format_args!("invalid hex string: {}, expected 7 instead of {} characters", value, value.len())));
        }

        let component = |number: usize| {
            let slice = &value[2 * number + 1 ..= 2 * number + 2];
            u8::from_str_radix(slice, 16)
                .map_err(|_| E::custom(format_args!("invalid hex string: {}, invalid digit in {}", value, slice)))
        };
...
mod tests {
    ...
    fn test_invalid_digit() {
        ...
        assert_eq!(error.to_string(), "invalid hex string: #12345g, invalid digit in 5g");
    }
    ...
    fn test_too_few_digits() {
        ...
        let error = color(deserializer).unwrap_err();
        assert_eq!(error.to_string(), "invalid hex string: #12, expected 7 instead of 3 characters");
    }
    ...
    fn test_too_many_digits() {
        ...
        let error = color(deserializer).unwrap_err();
        assert_eq!(error.to_string(), "invalid hex string: #1234567, expected 7 instead of 8 characters");
    }
